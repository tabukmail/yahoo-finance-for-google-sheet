...
// weekly/monthly bars are timestamped at midnight UTC (start of the trading
// day in US Eastern); shifting that by the script's own time zone can cross
// back over midnight into the previous (weekend) day, so those intervals use
// the UTC calendar date as-is instead of a time-zone-shifted conversion
var isDateOnlyInterval = DATE_ONLY_INTERVALS.indexOf(interval) != -1;
var timeStampArr = timeStamp.map(function(ts){ return timeConverter(ts, isDateOnlyInterval); });
